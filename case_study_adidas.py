...
html_title = """"
<style>.title-test {font-weight"bold;
padding:5px;
border-radius:6px
}
</style>
...
with col4:
    fig = px.bar(df, "Retailer", y= "Total Sales", labels= {"Total Sales" : "Total Sales {$}"},
...
with dwn1:
    st.download_button('Get data', data= data.to_csv().encode('utf-8'),
                       file_name="Retailer Sales.csv", mime="test/csv")
...
with dwn2:
    st.download_button('Get data', data= result.to_csv().encode('utf-8'),
                       file_name="Monthly Sales.csv", mime="test/csv")
st.divider()
...
with dwn3:
    st.download_button('Get data', data= result1.to_csv().encode('utf-8'),
                       file_name="Sales by units sold.csv", mime="test/csv")
st.divider()
...
with dwn4:
    st.download_button('Get data', data= result2.to_csv().encode('utf-8'),
                       file_name="Sales by Region.csv", mime="test/csv")
...
with dwn5:
    st.download_button('Get Raw data', data= df.to_csv().encode('utf-8'),
                       file_name="Sales Raw Data.csv", mime="test/csv")
